...
void func_8000559C(void) {
    if (gSoftResetLineNum <= 15) {
        if (gSoftResetLineNum == 0) {
            u16 color = Display_GetBorderColor();
        ...
        Display_SetBorderColor(((((gSoftResetRed * (15 - gSoftResetLineNum)) >> 4) << 11) |
...
void SoftReset_Thread(void* unused) {
    ...
    Sched_RegisterClient(&D_800A6D20);

    // thread loop
    while (1) {
        SchedClient_WaitMsg(&D_800A6D20.thread); // hangs here until gets a soft reset.
        if (D_800A62E0.unk_A38 == 0) {
            continue;
        }

        if (D_800A6D20.gSoftResetLineNum < 0x10U) {
            UnkStruct4* temp_v0 = Display_GetVBlanks();
            if (temp_v0 != NULL) {
            ...
            } else if (Display_IsShowingBorder() != 0) {
                func_8000559C();
            } else {
...
void SoftReset_CreateThread(void) {
    SchedClient_Init(&D_800A6D20.thread, 2, 1);
